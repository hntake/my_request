...
class Home extends StatefulWidget {
  ...
  Home({Key? key, this.updatedData, this.modeData}) :
        flutterTts = FlutterTts(), // flutterTtsをコンストラクタ内で初期化
        super(key: key);
...
class _HomeState extends State<Home> {
  ...
  Future<void> initializeData() async {
    ...
    setState(() {
      ...
      if (widget.updatedData != null) {
        ...
        print('Saved message3: ${widget.updatedData?['message3'] ?? 'N/A'}');
        ...
        body: Center(
          child: Column(
            ...
            children: <Widget>[
              GestureDetector(
                onTap: () {
                  readAloud1(messages:['${preferences.getString('message1').toString()}', '${preferences.getString('message2').toString()}']);
                  Navigator.push(
                      context, 
                      MaterialPageRoute(
  ...
  Future<void> readAloud1({List<String> messages = const ["message1", "message2"]}) async {
    final flutterTts = FlutterTts();
    final prefs = await SharedPreferences.getInstance();
    String? messageText = await prefs.getString('message1');
    ...
    print('Starting to read aloud:${preferences.getString('message1').toString()}');
                            ...
                            children: <Widget>[
                              if ('${preferences.getString('message1')
                                ...
                                Padding(
                                  ...
                                  child: Text('${preferences.getString(
                                      'message1')
                                      .toString()}'),
                                ),
...
class PainPage extends StatefulWidget {
  ...
  @override
  _PainPageState createState() => _PainPageState(preferences); // ここでpreferencesを渡す
}
class _PainPageState extends State<PainPage> {
  bool isLoading = false;
  late SharedPreferences preferences; // ここで preferences フィールドを宣言

  _PainPageState(this.preferences); // コンストラクタでpreferencesを受け取る
              ...
              child: Column(
                children: <Widget>[
                  Container(
                    ...
                    child: Column(
                      children: <Widget>[
                        if ('${preferences.getString('message1').toString()}' !=
                            null)
                          Padding(
                            padding: EdgeInsets.all(8.0),
                        ...
                        Image.asset('images/angry.png', width: 30),
                        if ('${preferences.getString('message2').toString()}' !=
                            null)
                          Padding(
                            padding: EdgeInsets.all(8.0),
